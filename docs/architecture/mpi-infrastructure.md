...
| MCP server config | `.mcp.json` | Context7, GitHub, Heroku, AWS, DigitalOcean, Honeybadger (gitignored; `.mcp.json.example` provided) |
...
| **Avails** | `mpimedia/avails_server` | Central data repository | Rails, PostgreSQL, Elasticsearch |
...
Workflow commands follow the 5-stage development lifecycle (see `docs/standards/development-lifecycle.md`):

**Lifecycle Commands:**

| Command | Stage | Purpose |
|---------|-------|---------|
| `assess.md` | 1. Assess | Analyze issue, research codebase, propose options |
| `cplan.md` | 2. Plan | Create implementation plan with testing + agent strategy |
| `impl.md` | 3. Implement | Execute plan, self-review, create PR |
| `verify.md` | 4. Verify | Self-review PR against plan |
| `final.md` | 5. Deliver | Post SOW on PR, link on Issue, prepare for merge |
| `rtr.md` | (Deliver) | Read and respond to PR review comments |

**Supporting Commands:**

| Command | Purpose |
|---------|---------|
| `orch.md` | Design multi-agent orchestration (when `/cplan` recommends parallel) |
| `explore.md` | Deep-dive into a codebase area |
| `compare.md` | Diff standards against another MPI repo |
| `dep-review.md` | Review Dependabot/dependency update PRs |
| `db-health.md` | Run database health diagnostics |
| `memory-review.md` | Audit auto-memory and recommend maintenance |

All commands are top-level files in `.claude/commands/`.

### MCP Servers

MCP (Model Context Protocol) servers give ACs access to external tools and data. Configuration lives in `.mcp.json` (gitignored). A `.mcp.json.example` template is provided. See `docs/architecture/mcp-integration-audit.md` for the full audit and rationale.

The MCP configuration has been optimized to include only servers that provide unique value over CLI tools:

| Server | Purpose | How to Get Key |
|--------|---------|----------------|
| **Cloudflare** (Code Mode) | Full Cloudflare API via 2 tools | 1Password vault |
| **Honeybadger** | Error monitoring across MPI projects | 1Password vault |

Servers removed in favor of CLI tools (lower context overhead, equivalent functionality):

| Removed Server | Replacement | Auth |
|----------------|-------------|------|
| Context7 | Context7 plugin (`claude plugin install context7@claude-plugins-official`) | Plugin handles auth |
| GitHub | `gh` CLI | `gh auth login` |
| Heroku | `heroku` CLI | `heroku login` |
| AWS | `aws` CLI | `aws configure` |
| DigitalOcean | `doctl` CLI | `doctl auth init` |
...
2. AC analyzes issue, posts assessment + options
3. HC sends to Codex, chooses option

── Stage 2: Plan (/cplan NNN) ──
4. AC creates plan (testing strategy + agent strategy)
5. HC sends to Codex, approves plan

── Stage 3: Implement (/impl NNN) ──
6. AC implements, self-reviews, creates PR

── Stage 4: Verify (/verify NNN) ──
7. AC self-reviews PR against plan

── Stage 5: Deliver (/final NNN) ──
8. Reviewer (Codex and/or Copilot) reviews PR
9. AC addresses review (/rtr NNN)
10. AC posts SOW on PR, links on Issue
11. HC merges PR --> Issue closed
```
...
.mcp.json.example                      # Copy, then adjust per-repo profile (see mcp-integration-audit.md)
```
...
# Create MCP config (Cloudflare + Honeybadger only — see mcp-integration-audit.md)
bin/setup-mcp
# Or manually: cp .mcp.json.example .mcp.json and fill in keys
...
|   +-- commands/
|   |   +-- assess.md                      # Stage 1: Assess
|   |   +-- cplan.md                       # Stage 2: Plan
|   |   +-- impl.md                        # Stage 3: Implement
|   |   +-- verify.md                      # Stage 4: Verify
|   |   +-- final.md                       # Stage 5: Deliver
|   |   +-- rtr.md                         # Respond to review
|   |   +-- orch.md                        # Multi-agent orchestration
|   |   +-- explore.md                     # Codebase exploration
|   |   +-- compare.md                     # Cross-repo comparison
|   |   +-- dep-review.md                  # Dependency review
|   |   +-- db-health.md                   # Database health
...
+-- .mcp.json                              # MCP server config (gitignored, Cloudflare + Honeybadger)
+-- .mcp.json.example                      # MCP template (optimized profile)
+-- docs/
    +-- architecture/
    |   +-- overview.md                    # Project architecture
    |   +-- agent-workflow.md              # AC workflow + multi-agent patterns
    |   +-- mcp-integration-audit.md       # MCP/plugin/CLI audit and per-repo profiles
...
```bash
# Lifecycle (see docs/standards/development-lifecycle.md)
/assess NNN                  # Stage 1: Analyze issue, post assessment
/cplan NNN                   # Stage 2: Create plan (testing + agent strategy)
/impl NNN                    # Stage 3: Implement, self-review, create PR
/verify NNN                  # Stage 4: Self-review PR against plan
/final NNN                   # Stage 5: Post SOW, prepare for merge
/rtr NNN                     # Respond to PR review comments
