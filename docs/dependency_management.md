...
### Kamal Gem Updates (Extra Step Required)
When the `kamal` gem is updated, the kamal-proxy on the servers will be out of sync and **deploys will fail**. After merging a Kamal gem update, reboot the proxy on all environments:

```bash
bin/kamal proxy reboot              # Production
bin/kamal proxy reboot -d staging   # Staging
```
